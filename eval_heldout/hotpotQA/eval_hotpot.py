...
parser = argparse.ArgumentParser(description="Test tgi checkpoint on hotpotQA.")
parser.add_argument(
    "--agent_name", type=str, help="Name of the agent.", default="React"
)
parser.add_argument(
    "--llm_name", type=str, help="Name of the llm", default="gpt-3.5-turbo"
)
parser.add_argument(
    "--max_context_len", type=int, help="Maximum context length", default=1700
)
...
def run_one_complex_level(args, level="easy"):
    print(llm_name)
    hotpot = joblib.load(f"src/data/{level}.joblib").reset_index(drop=True)
    agent_save_file = (
        f"execution_data/{args.min}-{args.max}/{level}_{agent_name}_{llm_name}.jsonl"
    )

    task_instructions = [
        (row["question"], row["answer"]) for _, row in hotpot.iterrows()
    ]
    ...
    agents = [
        agent_cls(ques, ans, llm, max_context_len) for ques, ans in task_instructions
    ]
